...
class TestRailScript {
    ...
    generateCases() {
        ...
        let onlineCases = this.testRail.getCases();
        if (!basePath) {
                ...
                if (its && its.length) {
                    ...
                    for (let it of its) {
                        ...
                        if (!caseId.length) {
                            ...
                            let localCase = itDescription.replace(` C${caseId}`, '');
                            onlineCases.map( (currentCase, index) => {
                            if(currentCase.id == caseId){
                                if(localCase !== currentCase.title){
                                    this.testRail.updateCaseById(caseId, localCase)
                                    console.log(`Case ${caseId} is updated`);
                                }
                            }
                            });
                        }
